...
GLuint loadSIF_custom(const char * imagepath){
	...
	unsigned int imageSize = (g_gl_height * g_gl_height * 4);
	GLubyte *data = (unsigned char*)malloc(imageSize);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, g_gl_width, g_gl_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
